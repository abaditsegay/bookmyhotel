...
public class SupervisorController {
    ...
    /**
     * Get supervisor dashboard overview - simplified version without circular
     * references
     */
     ...
     * Includes staff from both users table (created by hotel admin) and
    ...
    public ResponseEntity<?> getStaff() {
        try {
            ...
            for (User user : allUsers) {
                // Only include users from the same hotel or if hotelId is null (system-wide
                // access)
        ...
        try {
            ...
            List<Map<String, Object>> tasksData = allTasks.stream()
                    .map(task -> {
                        ...
                        taskMap.put("priority", task.getPriority() != null ? task.getPriority().toString() : "NORMAL");
                        taskMap.put("description", task.getDescription() != null ? task.getDescription() : "");
                        taskMap.put("estimatedDuration",
                                task.getEstimatedDurationMinutes() != null
                                        ? task.getEstimatedDurationMinutes().intValue()
                                        : 0);
                        taskMap.put("actualDuration",
                                task.getActualDurationMinutes() != null ? task.getActualDurationMinutes().intValue()
                                        : 0);
    ...
    public ResponseEntity<?> getMaintenance(@RequestParam(defaultValue = "0") int page,
        ...
        try {
            ...
            // Page<MaintenanceRequest> maintenanceRequests =
            // maintenanceService.getAllMaintenanceRequests(tenantId, pageable);
    ...
    public ResponseEntity<?> createMaintenanceRequest(@RequestBody CreateMaintenanceRequest request) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED)
                    .body(Map.of("error",
                            "Maintenance request creation not yet implemented - MaintenanceService needed"));
             ...
             * String roomNumber = "Unknown";
             * try {
             * if (maintenanceRequest.getRoom() != null &&
             * maintenanceRequest.getRoom().getRoomNumber() != null) {
    ...
    public ResponseEntity<?> getStaffTasks(@PathVariable Long staffId,
        ...
        try {
            ...
            for (HousekeepingTask task : tasks) {
                ...
                taskMap.put("priority", task.getPriority() != null ? task.getPriority().toString() : "NORMAL");
                taskMap.put("description", task.getDescription() != null ? task.getDescription() : "");
                taskMap.put("specialInstructions",
                        task.getSpecialInstructions() != null ? task.getSpecialInstructions() : "");
                taskMap.put("estimatedDuration",
                        task.getEstimatedDurationMinutes() != null ? task.getEstimatedDurationMinutes().intValue() : 0);
                taskMap.put("actualDuration",
                        task.getActualDurationMinutes() != null ? task.getActualDurationMinutes().intValue() : 0);
    ...
    public ResponseEntity<?> getStaffPerformance(@PathVariable Long staffId) {
        try {
            ...
            long inProgressTasks = allTasks.stream().filter(t -> t.getStatus() == HousekeepingTaskStatus.IN_PROGRESS)
                    .count();
            ...
            performance.put("completionRate",
                    allTasks.size() > 0 ? Math.round((completedTasks * 100.0 / allTasks.size()) * 100.0) / 100.0 : 0.0);
    ...
    public ResponseEntity<?> assignHousekeepingTask(@PathVariable Long taskId, @PathVariable Long staffId) {
        try {
            ...
            taskMap.put("taskType",
                    assignedTask.getTaskType() != null ? assignedTask.getTaskType().toString() : "UNKNOWN");
            taskMap.put("status", assignedTask.getStatus() != null ? assignedTask.getStatus().toString() : "UNKNOWN");
            taskMap.put("priority",
                    assignedTask.getPriority() != null ? assignedTask.getPriority().toString() : "NORMAL");
    ...
    public ResponseEntity<?> assignMaintenanceRequest(@PathVariable Long requestId, @PathVariable Long staffId) {
        try {
            ...
            // MaintenanceRequest assignedRequest =
            // maintenanceService.assignMaintenanceRequest(requestId, staffId, tenantId);

            return ResponseEntity.status(HttpStatus.NOT_IMPLEMENTED)
                    .body(Map.of("error",
                            "Maintenance request assignment not yet implemented - MaintenanceService needed"));
             ...
             * String roomNumber = "Unknown";
             * try {
             * if (assignedRequest.getRoom() != null &&
             * assignedRequest.getRoom().getRoomNumber() != null) {
