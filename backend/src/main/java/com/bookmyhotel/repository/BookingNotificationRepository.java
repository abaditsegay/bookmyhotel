...
public interface BookingNotificationRepository extends JpaRepository<BookingNotification, Long> {
    ...
    /**
     * Find all notifications for a specific hotel ordered by creation date (newest first)
     */
     ...
     */
    Page<BookingNotification> findByHotelIdAndTypeOrderByCreatedAtDesc(Long hotelId, NotificationType type, Pageable pageable);
    
    /**
     * Find notifications by hotel ID, type and status
     */
    Page<BookingNotification> findByHotelIdAndTypeAndStatusOrderByCreatedAtDesc(Long hotelId, NotificationType type, NotificationStatus status, Pageable pageable);
    ...
    void deleteOldNotifications(@Param("cutoffDate") java.time.LocalDateTime cutoffDate, @Param("status") NotificationStatus status);
}