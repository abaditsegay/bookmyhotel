...
public class HousekeepingTask {
    ...
    public HousekeepingTask(Room room, HousekeepingTaskType taskType, HousekeepingTaskStatus status, 
                           TaskPriority priority, String description, String tenantId) {
        this.room = room;
            ...
            switch (status) {
                ...
                case COMPLETED_WITH_ISSUES:
                    if (this.completedAt == null) {
                        ...
                        if (this.startedAt != null) {
                            this.actualDurationMinutes = (int) java.time.Duration.between(this.startedAt, now).toMinutes();
                        }
                    }
                    break;
                default:
    ...
    public int getEfficiencyScore() {
        ...
        if (ratio <= 0.8) return 5; // Completed 20% faster
        if (ratio <= 1.0) return 4; // Completed on time or faster
        if (ratio <= 1.2) return 3; // 20% over estimate
        if (ratio <= 1.5) return 2; // 50% over estimate
