...
public class ProductService {
    ...
    public ProductResponse createProduct(Long hotelId, ProductRequest request) {
        ...
        if (request.getSku() != null && !request.getSku().trim().isEmpty()) {
            if (productRepository.existsByHotelIdAndSku(hotelId, request.getSku())) {
                throw new BadRequestException("Product with SKU '" + request.getSku() + "' already exists for this hotel");
            }
        }
    ...
    private String generateSku(Hotel hotel, Product product) {
        ...
        String categoryPrefix = product.getCategory().name().substring(0, Math.min(3, product.getCategory().name().length()));
