...
public class MicrosoftGraphEmailService {
    ...
    private String getAccessToken() {
        if (!isConfigured()) {
            throw new IllegalStateException(
                    "Microsoft Graph OAuth2 is not configured. Please set environment variables: MICROSOFT_GRAPH_CLIENT_ID, MICROSOFT_GRAPH_TENANT_ID, MICROSOFT_GRAPH_CLIENT_SECRET");
        }
        ...
        if (accessToken == null || System.currentTimeMillis() >= tokenExpiryTime) {
            try {
                ...
                if (tokenResponse != null && tokenResponse.containsKey("access_token")) {
                    ...
                    tokenExpiryTime = System.currentTimeMillis() + (expiresIn * 1000L) - 60000; // Refresh 1 minute
                                                                                                // early
    ...
    private Map<String, Object> buildEmailMessage(String to, String subject, String htmlContent) {
        ...
        // Set sender address
        Map<String, Object> fromEmailAddress = new HashMap<>();
        fromEmailAddress.put("address", fromEmail);
        fromEmailAddress.put("name", "BookMyHotel");

        Map<String, Object> fromSender = new HashMap<>();
        fromSender.put("emailAddress", fromEmailAddress);
        message.put("from", fromSender);
