...
public class BookingService {
    ...
    public BookingResponse cancelBooking(Long reservationId) {
         ...
         * );
         * } catch (Exception e) {
         * logger.warn("Failed to record booking cancellation in history: {}",
         * e.getMessage());
         * }
         */
    ...
    private BigDecimal calculateTotalAmountByRoomType(Long hotelId, RoomType roomType, BookingRequest request) {
        ...
        if (pricePerNight == null) {
            ...
            throw new IllegalStateException("Room pricing not configured for hotel " + hotelId + " and room type "
                    + roomType + ". Please configure pricing before accepting bookings.");
        }
    ...
    public BookingResponse convertToBookingResponse(Reservation reservation) {
        ...
        if (reservation.getPaymentMethod() != null) {
            response.setPaymentProvider(reservation.getPaymentMethod().toUpperCase());
        }

        if (paymentInitiationResponse != null) {
            response.setPaymentProvider(paymentInitiationResponse.getPaymentProvider());
            response.setPaymentUrl(paymentInitiationResponse.getPaymentUrl());
            response.setPaymentQrCode(paymentInitiationResponse.getQrCode());
            response.setPaymentInstructions(paymentInitiationResponse.getInstructions());
            response.setPaymentExpiresAt(paymentInitiationResponse.getExpiresAt());

            if ((response.getPaymentReference() == null || response.getPaymentReference().isBlank())
                    && paymentInitiationResponse.getTransactionId() != null) {
                response.setPaymentReference(paymentInitiationResponse.getTransactionId());
            }
        }
    ...
    /**
     * Find booking by confirmation number across all tenants (for public booking
     * search)
     */
    ...
    public BookingResponse findByEmailAndLastNamePublic(String email, String lastName) {
        ...
        User user = userRepository.findByEmailPublic(email)
                .orElseThrow(
                        () -> new ResourceNotFoundException("No booking found for the provided email and last name"));

        // Find the most recent reservation for this user with matching last name using
        // public search
    ...
    public BookingModificationResponse modifyBooking(BookingModificationRequest request) {
        try {
            ...
            Reservation reservation = reservationRepository
                    .findByConfirmationNumberPublic(request.getConfirmationNumber())
            ...
            if (request.getNewCheckInDate() != null || request.getNewCheckOutDate() != null) {
                LocalDate newCheckIn = request.getNewCheckInDate() != null ? request.getNewCheckInDate()
                        : reservation.getCheckInDate();
                LocalDate newCheckOut = request.getNewCheckOutDate() != null ? request.getNewCheckOutDate()
                ...
                BigDecimal oldTotal = calculateSubtotalAmount(reservation, oldNights);
                BigDecimal newTotal = calculateSubtotalAmount(reservation, newNights);
             ...
             * );
             * } catch (Exception e) {
             * logger.warn("Failed to record booking modification in history: {}",
             * e.getMessage());
             * }
             */
    ...
    public BookingModificationResponse modifyCustomerBooking(Long reservationId, BookingModificationRequest request,
        ...
        try {
            ...
            if (request.getNewRoomType() != null && !request.getNewRoomType().trim().isEmpty()) {
                ...
                LocalDate effectiveCheckIn = request.getNewCheckInDate() != null ? request.getNewCheckInDate()
                        : reservation.getCheckInDate();
                LocalDate effectiveCheckOut = request.getNewCheckOutDate() != null ? request.getNewCheckOutDate()
                        : reservation.getCheckOutDate();
     ...
     */
    private BookingModificationResponse validateModificationRequest(BookingModificationRequest request,
            Reservation reservation) {
        // Check if booking can be modified
        if (reservation.getStatus() == ReservationStatus.CHECKED_IN ||
                reservation.getStatus() == ReservationStatus.CHECKED_OUT ||
    ...
    private BigDecimal calculateModificationCosts(BookingModificationRequest request, Reservation reservation) {
        ...
        if (request.getNewRoomType() != null) {
            ...
            targetRoom = roomRepository.findFirstAvailableRoomOfTypePublic(
                    ...
                    request.getNewRoomType(),
                    newCheckIn,
                    newCheckOut).orElse(reservation.getRoom());
        }
        ...
        try {
            BigDecimal modificationFeeRate = hotelPricingConfigService
     ...
     */
    private Room findAvailableRoomOfType(String roomType, LocalDate checkIn, LocalDate checkOut,
     ...
     */
    private boolean isRoomAvailableForModification(Long roomId, LocalDate checkIn, LocalDate checkOut,
            Long excludeReservationId) {
    ...
    /**
     * Calculate subtotal amount (tax-exclusive) for a given number of nights
     */
    private BigDecimal calculateSubtotalAmount(Reservation reservation, long nights) {
        ...
        logger.debug("Calculated subtotal amount for reservation {} for {} nights: base={}",
                reservation.getId(), nights, baseAmount);
    ...
    private String resolvePaymentPhoneNumber(String mobileNumber, String guestPhone) {
    ...
    private void ensureRealPaymentGatewayEnabled() {
        if (!systemSettingsService.isRealPaymentGatewayEnabled()) {
            throw new BookingException("Live Ethiopian payment gateway is disabled for this environment");
        }
    }
    ...
    private BigDecimal getSeasonalMultiplier(Long hotelId, LocalDate checkInDate, LocalDate checkOutDate) {
        ...
        // Simple seasonal logic: December (12), January (1), February (2) are peak
        // months
        boolean isPeakSeason = isPeakSeasonPeriod(checkInDate, checkOutDate);

        if (isPeakSeason) {
            logger.debug("Peak season detected for hotel {}, applying multiplier: {}", hotelId,
                    config.getPeakSeasonMultiplier());
            return config.getPeakSeasonMultiplier();
        } else {
            logger.debug("Off season detected for hotel {}, applying multiplier: {}", hotelId,
                    config.getOffSeasonMultiplier());
