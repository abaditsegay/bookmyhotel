...
# AWS RDS Migration Script: Convert room_id to room_number in housekeeping_tasks
# This script only handles the room_id → room_number conversion
...
# Migration file
MIGRATION_FILE="aws-room-id-to-room-number-migration.sql"
BACKUP_FILE="housekeeping_tasks_room_backup_$(date +%Y%m%d_%H%M%S).sql"

print_status "Starting AWS RDS migration: room_id → room_number conversion"
...
# Show current room-related columns
print_step "Current housekeeping_tasks room-related columns:"
"$MYSQL_CMD" -h"$RDS_HOST" -P"$RDS_PORT" -u"$RDS_USERNAME" -p"$RDS_PASSWORD" "$RDS_DATABASE" \
    -e "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'housekeeping_tasks' AND COLUMN_NAME IN ('room_id', 'room_number') ORDER BY COLUMN_NAME;"
...
# Apply migration
print_step "Applying room_id → room_number migration..."
...
echo ""
print_status "Updated room-related columns:"
"$MYSQL_CMD" -h"$RDS_HOST" -P"$RDS_PORT" -u"$RDS_USERNAME" -p"$RDS_PASSWORD" "$RDS_DATABASE" \
    -e "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'housekeeping_tasks' AND COLUMN_NAME IN ('room_id', 'room_number') ORDER BY COLUMN_NAME;"
...
"$MYSQL_CMD" -h"$RDS_HOST" -P"$RDS_PORT" -u"$RDS_USERNAME" -p"$RDS_PASSWORD" "$RDS_DATABASE" \
    -e "SELECT id, task_type, room_number, assigned_user_id, status, created_at 
...
echo "✅ Added room_number column (VARCHAR(20))"
echo "✅ Migrated data from room_id to room_number"
echo "✅ Removed room_id column and constraints"
echo "✅ Added performance index for room_number"
echo ""
print_status "Your AWS database now uses room_number instead of room_id!"